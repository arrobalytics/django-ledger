...
from django_ledger.report.core import (BaseReportSupport,
...
class CashFlowStatementReport(BaseReportSupport):
        ...
        **kwargs,
        ...
        if not io_digest.has_cash_flow_statement():
            raise PDFReportValidationError(
                "IO Digest does not have income statement information."
            )
        super().__init__(
            *args, io_digest=io_digest, report_subtitle=report_subtitle, **kwargs
        )
        self.TABLE_HEADERS = {
            "empty_1": {"title": "", "spacing": 165, "align": "L"},
            "total": {
    ...
    def print_amount(self, amt, zoom=0):
        self.set_x(178)
        self.set_font(family=self.FONT_FAMILY, size=self.FONT_SIZE + zoom)
        self.cell(
            w=20,
            h=5,
            markdown=True,
    ...
    def print_starting_net_income(self):
        net_income = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["operating"][
    ...
    def print_cash_from_operating(self):
        ...
        self.set_x(40)
        self.print_section_title(
            title="Non-cash Charges to Non-current Accounts", style="", zoom=2
        )
        self.ln(5)

        self.set_x(50)
        self.print_section_title(
            title="Depreciation & Amortization of Assets",
            style="",
            zoom=0,
            align="R",
            w=100,
        )
        amt = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["operating"][
        ...
        self.set_x(50)
        self.print_section_title(
            title="Gain/Loss Sale of Assets", style="", zoom=0, align="R", w=100
        )
        amt = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["operating"][
        ...
        self.set_x(40)
        self.print_section_title(
            title="Non-cash Charges to Current Accounts", style="", zoom=2
        )
        self.ln(5)

        self.set_x(50)
        self.print_section_title(
            title="Accounts Receivable", style="", zoom=0, align="R", w=100
        )
        amt = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["operating"][
        ...
        self.set_x(50)
        self.print_section_title(
            title="Inventories", style="", zoom=0, align="R", w=100
        )
        amt = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["operating"][
        ...
        self.set_x(50)
        self.print_section_title(
            title="Accounts Payable", style="", zoom=0, align="R", w=100
        )
        amt = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["operating"][
        ...
        self.set_x(50)
        self.print_section_title(
            title="Other Current Assets", style="", zoom=0, align="R", w=100
        )
        amt = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["operating"][
        ...
        self.set_x(50)
        self.print_section_title(
            title="Other Current Liabilities", style="", zoom=0, align="R", w=100
        )
        amt = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["operating"][
        ...
        self.print_hline()
        self.print_section_title(
            title="Net Cash Provided by Operating Activities", zoom=1, w=100, style="B"
        )
        amt = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["net_cash_by_activity"][
            "OPERATING"
        ]
    ...
    def print_cash_from_financing(self):
        ...
        for _, sec in cf_section.items():
            self.set_x(50)
            self.print_section_title(
                title=sec["description"], style="", zoom=0, align="R", w=100
            )
            self.print_amount(amt=sec["balance"])
            self.ln(5)

        self.print_hline()
        self.print_section_title(
            title="Net Cash Provided by Financing Activities", zoom=1, w=100, style="B"
        )
        amt = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["net_cash_by_activity"][
            "FINANCING"
        ]
    ...
    def print_cash_from_investing(self):
        ...
        self.print_hline()
        self.print_section_title(
            title="Net Cash Provided by Investing Activities", zoom=1, w=100, style="B"
        )
        amt = self.IO_DIGEST.IO_DATA["cash_flow_statement"]["net_cash_by_activity"][
            "INVESTING"
        ]
    ...
    def print_net_cash_flow(self):
        ...
        self.print_amount(
            amt=self.IO_DIGEST.IO_DATA["cash_flow_statement"]["net_cash"], zoom=3
        )
        self.ln(5)
    ...
    def get_pdf_filename(
        ...
        if from_dt:
            from_dt = from_dt.strftime(
                dt_strfmt if dt_strfmt else self.IO_DIGEST.get_strftime_format()
            )
        else:
            from_dt = self.IO_DIGEST.get_from_datetime(fmt=dt_strfmt, as_str=True)

        if to_dt:
            to_dt = to_dt.strftime(
                dt_strfmt if dt_strfmt else self.IO_DIGEST.get_strftime_format()
            )
        else:
