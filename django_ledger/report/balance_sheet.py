...
class BalanceSheetReport(BaseReportSupport):
    ...
    def print_section_data(self, section_data):
        for r, d in section_data["roles"].items():
            ...
            self.set_x(15)
            self.cell(w=self.get_string_width(d["role_name"]), h=2, txt=d["role_name"])
            self.ln(h=5)
            ...
            for acc in d["accounts"]:
                self.cell(
                    w=self.TABLE_HEADERS["role"]["width"], h=account_height, txt=""
                )
                self.cell(
            ...
            self.cell(w=20, h=2, txt="Total {r}".format(r=d["role_name"]))
            ...
            self.cell(
                ...
                align="R",
                txt="{s}{tot}".format(
                    s=currency_symbol(), tot=currency_format(d["total_balance"])
                ),
            )
            self.ln(3)
            self.print_hline()
    ...
    def print_liabilities(self):
        ...
        if section_data:
            ...
            self.cell(
                ...
                h=2,
                align="R",
                txt="Total Liabilities: {s}{tot}".format(
                    s=currency_symbol(),
                    tot=currency_format(section_data["total_balance"]),
                ),
            )
    ...
    def print_equity(self):
        ...
        self.set_font(self.FONT_FAMILY, style="B", size=self.FONT_SIZE)
        self.cell(w=20, h=2, txt="Retained Earnings")
        self.set_x(180)
        self.cell(
            w=20,
            h=2,
        ...
        self.cell(
            ...
            h=2,
            align="R",
            txt="Total Equity: {s}{tot}".format(
                s=currency_symbol(), tot=currency_format(bs_data["equity_balance"])
            ),
        )
        self.ln(h=10)
        ...
        self.cell(
            ...
            h=2,
            align="R",
            txt="Total Liabilities + Equity: {s}{tot}".format(
                s=currency_symbol(),
                tot=currency_format(bs_data["liabilities_equity_balance"]),
            ),
        )
        self.ln(h=5)

    def get_pdf_filename(
        self, to_dt: Optional[Union[datetime, date]] = None, dt_strfmt: str = "%Y%m%d"
    ):
        if to_dt:
