...
class BaseReportSupport(FPDF):
    ...
    def header(self):
        # # Report Type
        self.set_font(family=self.FONT_FAMILY, size=self.FONT_SIZE + 2)
        w = self.get_string_width(self.get_report_name())
        self.set_x((self.PAGE_WIDTH - w) / 2)
        self.cell(w, 3, self.get_report_name(), ln=1)

        # Report Title
        self.set_font(family=self.FONT_FAMILY, size=self.FONT_SIZE + 6, style="B")
        ...
        if self.REPORT_SUBTITLE:
            self.set_font(family=self.FONT_FAMILY, size=self.FONT_SIZE, style="UI")
        ...
        # Period
        self.set_font(family=self.FONT_FAMILY, size=self.FONT_SIZE - 1, style="I")
        ...
        self.set_x((self.PAGE_WIDTH - w) / 2)
        self.cell(w=w, h=5, txt=period, new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    ...
    def get_report_title(self):
        ...
        raise PDFReportValidationError(
            "get_report_title() not implemented for"
            f" IO_MODEL {self.IO_DIGEST.IO_MODEL.__class__.__name__}"
        )

    def get_report_name(self):
        raise NotImplementedError(
            f"Must define REPORT_TYPE on {self.__class__.__name__}"
        )
    ...
    def print_section_title(self, title, style="BU", zoom=4, w=160, align="L"):
        self.set_font(family=self.FONT_FAMILY, style=style, size=self.FONT_SIZE + zoom)
        # Assets title...
        self.cell(w=w, h=6, txt=title, align=align)
        self.set_default_font()
    ...
    def footer(self):
        ...
        self.image(
            self.get_report_footer_logo_path(),
            w=30,
            x=(200 - 30) / 2,
