...
class AuthTest(DjangoLedgerBaseTest):
    TEST_URLS = [
        ...
        reverse("django_ledger:entity-dashboard", kwargs={"entity_slug": "fake-123"}),
    ]
    ...
    def test_login_form(self):
        ...
        self.assertContains(
            response, text='id="djl-el-login-form"', status_code=200, count=1
        )
        self.assertContains(
            response,
            text='id="djl-el-login-form-username-field"',
            status_code=200,
            count=1,
        )
        self.assertContains(
            response,
            text='id="djl-el-login-form-password-field"',
            status_code=200,
            count=1,
        )
