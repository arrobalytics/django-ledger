...
class AccountModelTests(DjangoLedgerBaseTest):
    ...
    def setUp(self):
        self.resolve_url_patterns(url_patterns=account_urls)
    ...
    def test_account_create(self):
        ...
        self.assertEqual(
            response.status_code, 200, msg="Fail to GET Account Create page."
        )
        ...
        self.assertTrue(
            ...
            .with_codes(codes=NEW_ACCOUNT_CODE)
            .exists()
        )
        ...
        self.assertContains(
            response_create,
            "Account with this Chart of Accounts and Account Code already exists",
        )

    def test_account_activation(self):
        entity_model: EntityModel = self.get_random_entity_model()
        account_model: AccountModel = self.get_random_account(
            entity_model=entity_model, active=True
        )
    ...
    def test_account_lock(self):
        entity_model: EntityModel = self.get_random_entity_model()
        account_model: AccountModel = self.get_random_account(
            entity_model=entity_model, active=True, locked=False
        )
