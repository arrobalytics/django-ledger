...
class ChartOfAccountsTests(DjangoLedgerBaseTest):
    def test_no_default_coa(self):
        ...
        self.assertFalse(
            entity_model.has_default_coa(), msg="New entities do not had default coa"
        )

        with self.assertRaises(
            EntityModelValidationError, msg="New entities do not have default coa"
        ):
            entity_model.get_default_coa()

        self.assertEqual(
            entity_model.get_default_coa(raise_exception=False),
            None,
    ...
    def test_set_default_coa(self):
        ...
        coa_model = entity_model.create_chart_of_accounts(
            coa_name="Now CoA For Testing", assign_as_default=False
        )
