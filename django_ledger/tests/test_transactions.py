...
from django_ledger.forms.transactions import (
    TransactionModelForm, TransactionModelFormSet,
    get_transactionmodel_formset_class)
from django_ledger.io.io_core import get_localdate
from django_ledger.models import (AccountModel, EntityModel, JournalEntryModel,
                                  JournalEntryValidationError, LedgerModel,
                                  TransactionModel,
...
class TransactionModelFormTest(DjangoLedgerBaseTest):
    def test_valid_data(self):
        ...
        account_model = (
            str(
                self.get_random_account(
                    entity_model=entity_model, balance_type="credit"
                ).uuid
            ),
        )
        ...
        self.assertTrue(
            form.is_valid(), msg=f"Form is invalid with error: {form.errors}"
        )
    ...
    def test_invalid_tx_type(self):
        ...
        self.assertFalse(
            form.is_valid(), msg="tx_type other than credit / debit shouldn't be valid"
        )

    def test_blank_data(self):
        form = TransactionModelForm()
        self.assertFalse(
            form.is_valid(), msg="Form without data is supposed to be invalid"
        )
    ...
    def get_random_txs_formsets(
        ...
        if ledger_model:
            ...
            je_model: JournalEntryModel = (
                self.get_random_je(entity_model=entity_model, ledger_model=ledger_model)
                if not je_model
                else je_model
            )
            ...
            je_model = (
                JournalEntryModel.objects.for_entity(entity_model=entity_model)
                .annotate(txs_count=Count("transactionmodel"))
                .filter(txs_count__gt=0)
                .order_by("-timestamp")
                .first()
            )

        TransactionModelFormSet = get_transactionmodel_formset_class(
            journal_entry_model=je_model
        )
    ...
    def test_ledger_lock(self):
        ...
        with self.assertRaises(
            ...
            ledger_model.journal_entries.create(
                timestamp=get_localdate(), description="Test Journal Entry"
            )
...
class GetTransactionModelFormSetClassTest(DjangoLedgerBaseTest):
    def test_unlocked_journal_entry_formset(self):
        ...
        transaction_model_form_set = get_transactionmodel_formset_class(
            journal_entry_model=je_model
        )
    ...
    def test_locked_journal_entry_formset(self):
        ...
        self.assertTrue(
            je_model.is_locked(), msg="Journal Entry should be locked in this test case"
        )
        ...
        self.assertEqual(
            len(txs_formset),
            (je_model.transactionmodel_set.count()),  # Convert pairs to total count
