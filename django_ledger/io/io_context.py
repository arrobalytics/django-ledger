...
class IODigestContextManager:
    ...
    def is_entity_model(self) -> bool:
        return isinstance(self.IO_MODEL, lazy_loader.get_entity_model())

    def is_ledger_model(self) -> bool:
        return isinstance(self.IO_MODEL, lazy_loader.get_ledger_model())

    def is_unit_model(self) -> bool:
        return isinstance(self.IO_MODEL, lazy_loader.get_entity_unit_model())
    ...
    def get_account_data(self, key_func=None) -> Dict:
        if key_func:
            return {key_func(acc): acc for acc in self.IO_DATA["accounts"]}
        return {acc["account_uuid"]: acc for acc in self.IO_DATA["accounts"]}
    ...
    def get_financial_statements_data(self) -> Dict:
        return {
            ...
            "income_statement": self.get_income_statement_data()
            if self.has_income_statement()
            else None,
            "cash_flow_statement": self.get_cash_flow_statement_data()
            if self.has_cash_flow_statement()
            else None,
        }
