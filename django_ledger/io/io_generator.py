...
from django_ledger.io.roles import (ASSET_CA_CASH, ASSET_CA_INVENTORY,
                                    ASSET_CA_PREPAID, ASSET_CA_RECEIVABLES,
                                    COGS, EQUITY_CAPITAL, EXPENSE_OPERATIONAL,
                                    INCOME_OPERATIONAL,
                                    LIABILITY_CL_ACC_PAYABLE,
                                    LIABILITY_CL_DEFERRED_REVENUE)
from django_ledger.models import (BankAccountModel, BillModel,
                                  ChartOfAccountModel, CustomerModel,
                                  EntityModel, EntityUnitModel, EstimateModel,
                                  InvoiceModel, InvoiceModelValidationError,
                                  ItemModel, ItemTransactionModel,
                                  LoggingMixIn, TransactionModel,
                                  UnitOfMeasureModel, VendorModel)
from django_ledger.utils import (generate_random_item_id, generate_random_sku,
                                 generate_random_upc)
...
class EntityDataGenerator(LoggingMixIn):
    ...
    def __init__(
        ...
        ), "Must pass an instance of EntityModel or str"
        assert capital_contribution > 0, "Capital contribution must be greater than 0"

        if not FAKER_IMPORTED:
            raise ImproperlyConfigured(
                "Must install Faker library to generate random data."
            )

        if entity_model.admin != user_model:
            raise EntityModelValidationError(
                message=_(
                    f"User {user_model} must have admin privileges for entity model {entity_model}."
                )
            )
    ...
    def populate_entity(
        self, create_closing_entry: bool = False, force_populate: bool = False
    ):
        self.logger.info("Checking for existing transactions...")
        txs_qs = TransactionModel.objects.for_entity(entity_model=self.entity_model)
        ...
        self.account_models = self.entity_model.get_coa_accounts(
            order_by=("role", "code")
        )
    ...
    def get_next_timestamp(self, prev_timestamp: Union[date, datetime] = None) -> date:
        ...
        next_timestamp = prev_timestamp + timedelta(
            days=randint(self.MIN_DAYS_FORWARD, self.MAX_DAYS_FORWARD)
        )
    ...
    def create_vendors(self):
        ...
        vendor_models = [
            self.entity_model.create_vendor(
                vendor_model_kwargs={
                    "vendor_name": self.fk.name()
                    if random() > 0.7
                    else self.fk.company(),
                    "address_1": self.fk.street_address(),
                    "address_2": self.fk.building_number() if random() < 0.2 else None,
                    "city": self.fk.city(),
        ...
        self.vendor_models = VendorModel.objects.bulk_create(
            vendor_models, ignore_conflicts=True
        )
    ...
    def create_customers(self):
        ...
        self.customer_models = CustomerModel.objects.bulk_create(
            customer_models, ignore_conflicts=True
        )
    ...
    def create_bank_accounts(self):
        ...
        self.bank_account_models = BankAccountModel.objects.bulk_create(
            bank_account_models, ignore_conflicts=True
        )
    ...
    def create_uom_models(self):
        ...
        uom_models = [
            self.entity_model.create_uom(unit_abbr=abbr, name=name, commit=False)
    ...
    def create_estimate(self, date_draft: date):
        ...
        self.logger.info(
            f"Creating entity estimate {estimate_model.estimate_number}..."
        )
        ...
        estimate_items = [
            ItemTransactionModel(
                ...
                ce_unit_revenue_estimate=round(
                    random() * randint(80, 120) * (1 + 0.2 * random()), 2
                ),
        ...
        estimate_items = estimate_model.itemtransactionmodel_set.bulk_create(
            objs=estimate_items
        )
        ...
        if random() > 0.25:
            ...
            if random() > 0.50:
                ...
                estimate_model.mark_as_approved(
                    commit=True, date_approved=date_approved
                )
                if random() > 0.25:
                    date_completed = self.get_next_timestamp(date_approved)
                    estimate_model.mark_as_completed(
                        commit=True, date_completed=date_completed
                    )
                elif random() > 0.8:
                ...
                estimate_model.mark_as_canceled(
                    commit=True, date_canceled=date_canceled
                )
            ...
            if random() > 0.5:
                ...
                if random() > 0.25:
                    ...
                    bill_model = self.entity_model.create_bill(
                        ...
                        cash_account=choice(self.accounts_by_role[ASSET_CA_CASH]),
                        prepaid_account=choice(self.accounts_by_role[ASSET_CA_PREPAID]),
                        payable_account=choice(
                            self.accounts_by_role[LIABILITY_CL_ACC_PAYABLE]
                        ),
                        commit=True,
                    )
                    ...
                    if random() > 0.25:
                        ...
                        bill_model.mark_as_review(
                            commit=True, date_in_review=date_bill_review
                        )
                        if random() > 0.50:
                            bill_approve_date = self.get_next_timestamp(
                                date_bill_review
                            )
                            bill_model.mark_as_approved(
                                commit=True,
                            ...
                            if random() > 0.25:
                                bill_paid_date = self.get_next_timestamp(
                                    bill_approve_date
                                )
                                bill_model.mark_as_paid(
                                    user_model=self.user_model,
                                ...
                                if random() > 0.20:
                                    ...
                                    po_model.itemtransactionmodel_set.bulk_update(
                                        po_items, fields=["po_item_status", "updated"]
                                    )
                                    po_model.mark_as_fulfilled(
                                        date_fulfilled=date_fulfilled, commit=True
                                    )
    ...
    def create_invoice(self, date_draft: date):
        ...
        for i in range(randint(1, 10)):
            ...
            entity_unit = choice(self.entity_unit_models) if random() > 0.75 else None
            margin = Decimal(random() + 3.5)
            avg_cost = item_model.get_average_cost()
            if item_model.is_product():
                if (
                    item_model.inventory_received is not None
                    and item_model.inventory_received > 0.0
                ):
                ...
                if all([quantity > 0.00, unit_cost > 0.00]):
        ...
        invoice_items = invoice_model.itemtransactionmodel_set.bulk_create(
            invoice_items
        )
    ...
    def create_closing_entry(self):
        ...
        ce_model, ce_txs = self.entity_model.close_books_for_month(
            year=closing_date.year, month=closing_date.month, post_closing_entry=True
        )
