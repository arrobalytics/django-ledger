...
class AccountRoleIOMiddleware:
    def __init__(self, io_data: dict, by_period: bool = False, by_unit: bool = False):
        ...
        for c, l in roles_module.ROLES_DIRECTORY.items():
            for r in l:
                acc_list = list(
                    ...
                    if acc["role"] == getattr(roles_module, r)
                )
                ...
                if self.BY_PERIOD or self.BY_UNIT:
                    for acc in acc_list:
                        if self.BY_PERIOD:
                            ...
                            self.ROLES_BALANCES_BY_PERIOD[key][r] = sum(
                                acc["balance"]
                                for acc in acc_list
                                if all(
                                    [
...
class AccountGroupIOMiddleware:
    ...
    def digest(self):
        ...
        if self.BY_PERIOD:
            self.IO_DIGEST[
                self.GROUP_BALANCE_BY_PERIOD_KEY
            ] = self.GROUPS_BALANCES_BY_PERIOD
        if self.BY_UNIT:
            self.IO_DIGEST[
                self.GROUP_BALANCE_BY_UNIT_KEY
            ] = self.GROUPS_BALANCES_BY_UNIT
...
class JEActivityIOMiddleware:
    def __init__(self, io_data: dict, by_unit: bool = False, by_period: bool = False):
        self.DIGEST = io_data
        ...
        for act in JournalEntryModel.VALID_ACTIVITIES:
            ...
            if self.BY_PERIOD or self.BY_UNIT:
                for acc in acc_list:
                    if self.BY_PERIOD:
                        key = (acc["period_year"], acc["period_month"])
                        self.ACTIVITY_BALANCES_BY_PERIOD[key][act] = sum(
                            acc["balance"]
                            for acc in acc_list
                            if all(
                                [
        ...
        if "group_account" in self.IO_DATA:
            ...
            for bs_role, bs_role_data in bs_context.items():
                for acc_role, role_data in bs_role_data["roles"].items():
                    role_data["total_balance"] = sum(
                        a["balance"] for a in role_data["accounts"]
                    )
                    role_data["role_name"] = roles_module.ACCOUNT_LIST_ROLE_VERBOSE[
                        acc_role
                    ]

            bs_context["equity_balance"] = self.IO_DATA["group_balance"]["GROUP_EQUITY"]
            bs_context["retained_earnings_balance"] = self.IO_DATA["group_balance"][
                "GROUP_EARNINGS"
            ]
            bs_context["liabilities_equity_balance"] = self.IO_DATA["group_balance"][
                "GROUP_LIABILITIES_EQUITY"
            ]
        ...
        if "group_account" in self.IO_DATA:
            self.IO_DATA[self.IC_DIGEST_KEY] = {
                "operating": {
                    "revenues": [
                        acc
                        for acc in self.IO_DATA["group_account"]["GROUP_INCOME"]
                        if acc["role"] in roles_module.GROUP_IC_OPERATING_REVENUES
                    ],
                    "cogs": [
                        acc
                        for acc in self.IO_DATA["group_account"]["GROUP_COGS"]
                        if acc["role"] in roles_module.GROUP_IC_OPERATING_COGS
                    ],
                    "expenses": [
                        acc
                        for acc in self.IO_DATA["group_account"]["GROUP_EXPENSES"]
                        if acc["role"] in roles_module.GROUP_IC_OPERATING_EXPENSES
                    ],
                },
                "other": {
                    "revenues": [
                        acc
                        for acc in self.IO_DATA["group_account"]["GROUP_INCOME"]
                        if acc["role"] in roles_module.GROUP_IC_OTHER_REVENUES
                    ],
                    "expenses": [
                        acc
                        for acc in self.IO_DATA["group_account"]["GROUP_EXPENSES"]
                        if acc["role"] in roles_module.GROUP_IC_OTHER_EXPENSES
                    ],
                },
            }
            ...
            for activity, ic_section in self.IO_DATA[self.IC_DIGEST_KEY].items():
                for section, acc_list in ic_section.items():
                    for acc in acc_list:
                        acc["role_name"] = roles_module.ACCOUNT_LIST_ROLE_VERBOSE[
                            acc["role"]
                        ]
            ...
            self.IO_DATA[self.IC_DIGEST_KEY]["operating"]["gross_profit"] = sum(
                acc["balance"]
                for acc in chain.from_iterable(
                    [
            ...
            self.IO_DATA[self.IC_DIGEST_KEY]["operating"][
                "net_operating_revenue"
            ] = sum(
                acc["balance"]
                for acc in self.IO_DATA[self.IC_DIGEST_KEY]["operating"]["revenues"]
            )
            self.IO_DATA[self.IC_DIGEST_KEY]["operating"]["net_cogs"] = sum(
                acc["balance"]
                for acc in self.IO_DATA[self.IC_DIGEST_KEY]["operating"]["cogs"]
            )
            self.IO_DATA[self.IC_DIGEST_KEY]["operating"][
                "net_operating_expenses"
            ] = sum(
                acc["balance"]
                for acc in self.IO_DATA[self.IC_DIGEST_KEY]["operating"]["expenses"]
            )

            # OTHER INCOME....
            self.IO_DATA[self.IC_DIGEST_KEY]["other"]["net_other_revenues"] = sum(
                acc["balance"]
                for acc in self.IO_DATA[self.IC_DIGEST_KEY]["other"]["revenues"]
            )
            self.IO_DATA[self.IC_DIGEST_KEY]["other"]["net_other_expenses"] = sum(
                acc["balance"]
                for acc in self.IO_DATA[self.IC_DIGEST_KEY]["other"]["expenses"]
            )
            ...
            # NET INCOME...
            self.IO_DATA[self.IC_DIGEST_KEY]["net_income"] = self.IO_DATA[
                self.IC_DIGEST_KEY
            ]["operating"]["net_operating_income"]
            self.IO_DATA[self.IC_DIGEST_KEY]["net_income"] += self.IO_DATA[
                self.IC_DIGEST_KEY
            ]["other"]["net_other_income"]
        return self.IO_DATA
...
class CashFlowStatementIOMiddleware:
    ...
    def __init__(self, io_data: dict):
        ...
        self.CASH_ACCOUNTS = [
            a
            for a in self.IO_DATA["accounts"]
            if a["role"] == roles_module.ASSET_CA_CASH
        ]
    ...
    def check_io_digest(self):
        if AccountGroupIOMiddleware.GROUP_BALANCE_KEY not in self.IO_DATA:
            raise ValidationError("IO Digest must have groups for Cash Flow Statement")
    ...
    def operating(self):
        ...
        operating_activities["GROUP_CFS_OP_OTHER_CURRENT_LIABILITIES_ADJUSTMENT"] = {
            ...
            "balance": group_balances[
                "GROUP_CFS_OP_OTHER_CURRENT_LIABILITIES_ADJUSTMENT"
            ],
        }

        net_cash_by_op_activities = sum(
            i["balance"] for g, i in operating_activities.items()
        )
    ...
    def financing(self):
        ...
        financing_activities["GROUP_CFS_FIN_ISSUING_EQUITY"] = {
            ...
            "balance": sum(
                ...
                if a["activity"] == self.JE_MODEL.FINANCING_EQUITY
            ),
        }
        financing_activities["GROUP_CFS_FIN_DIVIDENDS"] = {
            ...
            "balance": sum(
                a["balance"]
                for a in self.CASH_ACCOUNTS
                if a["activity"] == self.JE_MODEL.FINANCING_DIVIDENDS
            ),
        }
        financing_activities["GROUP_CFS_FIN_ST_DEBT_PAYMENTS"] = {
            ...
            "balance": sum(
                a["balance"]
                for a in self.CASH_ACCOUNTS
                if a["activity"] == self.JE_MODEL.FINANCING_STD
            ),
        }
        financing_activities["GROUP_CFS_FIN_LT_DEBT_PAYMENTS"] = {
            ...
            "balance": sum(
                a["balance"]
                for a in self.CASH_ACCOUNTS
                if a["activity"] == self.JE_MODEL.FINANCING_LTD
            ),
        }
    ...
    def investing(self):
        ...
        investing_activities["GROUP_CFS_INVESTING_SECURITIES"] = {
            ...
            "balance": sum(
                a["balance"]
                for a in self.CASH_ACCOUNTS
                if a["activity"] == self.JE_MODEL.INVESTING_SECURITIES
            ),
        }
        investing_activities["GROUP_CFS_INVESTING_PPE"] = {
            ...
            "balance": sum(
                a["balance"]
                for a in self.CASH_ACCOUNTS
                if a["activity"] == self.JE_MODEL.INVESTING_PPE
            ),
        }
    ...
    def net_cash(self):
        self.IO_DATA[self.CFS_DIGEST_KEY]["net_cash"] = sum(
            [
                bal
                for act, bal in self.IO_DATA[self.CFS_DIGEST_KEY][
    ...
    def net_income(self):
        group_balances = self.IO_DATA[AccountGroupIOMiddleware.GROUP_BALANCE_KEY]
        self.IO_DATA[self.CFS_DIGEST_KEY]["net_income"] = group_balances[
