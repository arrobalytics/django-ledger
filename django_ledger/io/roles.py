...
GROUP_CFS_INVESTING_PPE = (
    GROUP_CFS_INV_PURCHASE_OR_SALE_OF_PPE + GROUP_CFS_INV_LTD_OF_PPE
)
...
GROUP_CFS_INVESTING_SECURITIES = (
    GROUP_CFS_INV_PURCHASE_OF_SECURITIES + GROUP_CFS_INV_LTD_OF_SECURITIES
)
...
ACCOUNT_ROLE_CHOICES = [
    (
        ...
        (
            ...
            (
                ASSET_PPE_BUILDINGS_ACCUM_DEPRECIATION,
                _("Buildings - Accum. Depreciation"),
            ),
            ...
            (
                ASSET_PPE_EQUIPMENT_ACCUM_DEPRECIATION,
                _("Equipment - Accum. Depreciation"),
            ),
            # Other Assets ...
            (ASSET_INTANGIBLE_ASSETS, _("Intangible Assets")),
            (
                ASSET_INTANGIBLE_ASSETS_ACCUM_AMORTIZATION,
                _("Intangible Assets - Accum. Amortization"),
            ),
...
ROLES_ORDER_ALL = list(
    chain.from_iterable(
        [ROLES_ORDER_ASSETS, ROLES_ORDER_LIABILITIES, ROLES_ORDER_CAPITAL]
    )
)

ACCOUNT_LIST_ROLE_ORDER = list(
    r[0] for r in chain.from_iterable([i[1] for i in ACCOUNT_CHOICES_NO_ROOT])
)
ACCOUNT_LIST_ROLE_VERBOSE = {
    r[0]: r[1] for r in chain.from_iterable([i[1] for i in ACCOUNT_CHOICES_NO_ROOT])
}

ROLE_TUPLES = sum(
    [[(r[0].lower(), s[0]) for s in r[1]] for r in ACCOUNT_ROLE_CHOICES], list()
)
...
def validate_roles(
    roles: Union[str, List[str]], raise_exception: bool = True
) -> Set[str]:
    """
        ...
        if r not in VALID_ROLES:
            if raise_exception:
                raise InvalidRoleError(
                    "{rls}) is invalid. Choices are {ch}".format(
                        ch=", ".join(VALID_ROLES), rls=r
                    )
                )
    return set(roles)
