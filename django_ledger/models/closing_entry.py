...
from django_ledger.models import (EntityModel, deprecated_entity_slug_behavior,
                                  lazy_loader)
...
class ClosingEntryModelManager(Manager):
    def get_queryset(self) -> ClosingEntryModelQuerySet:
        ...
        return qs.annotate(ce_txs_count=Count("closingentrytransactionmodel"))

    @deprecated_entity_slug_behavior
    def for_entity(
        self, entity_model: EntityModel | str | UUID = None, **kwargs
    ) -> ClosingEntryModelQuerySet:
...
class ClosingEntryModelAbstract(CreateUpdateMixIn, MarkdownNotesMixIn):
    ...
    ledger_model = models.OneToOneField(
        "django_ledger.LedgerModel", on_delete=models.CASCADE
    )
    ...
    objects = ClosingEntryModelManager.from_queryset(
        queryset_class=ClosingEntryModelQuerySet
    )()
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["entity_model", "closing_date"],
    ...
    def __str__(self):
        return (
            f"{self.__class__.__name__}: {self.entity_model.name} {self.closing_date}"
        )
    ...
    def migrate(self):
        ce_txs = (
            self.closingentrytransactionmodel_set.all()
            .select_related("account_model", "unit_model")
            .order_by(
                "tx_type",
                "account_model",
                "unit_model",
        ...
        if (
            len(ce_txs_sum)
            and ce_txs_sum[TransactionModel.DEBIT]
            != ce_txs_sum[TransactionModel.CREDIT]
        ):
            raise ClosingEntryValidationError(
        ...
        JournalEntryModel.objects.bulk_create(
            objs=chain([l for _, l in ce_txs_journal_entries.items()])
        )
        TransactionModel.objects.bulk_create(
            objs=chain.from_iterable([l for _, l in ce_je_txs.items()])
        )
    ...
    def create_entry_ledger(self, commit: bool = False):
        if self.ledger_model_id is None:
            ...
            if commit:
                self.save(update_fields=["ledger_model", "updated"])
    ...
    def mark_as_posted(
        self, commit: bool = False, update_entity_meta: bool = True, **kwargs
    ):
        ...
        self.posted = True
        if commit:
            self.save(update_fields=["posted", "ledger_model", "updated"])
    ...
    def get_mark_as_posted_message(self):
        return _(
            f"Are you sure you want to post Closing Entry dated {self.closing_date}?"
        )
    ...
    def get_mark_as_posted_url(self, entity_slug: Optional[str] = None) -> str:
        ...
        return reverse(
            viewname="django_ledger:closing-entry-action-mark-as-posted",
            kwargs={"entity_slug": entity_slug, "closing_entry_pk": self.uuid},
        )
    ...
    def get_mark_as_unposted_message(self):
        return _(
            f"Are you sure you want to unpost Closing Entry dated {self.closing_date}?"
        )
    ...
    def get_mark_as_unposted_url(self, entity_slug: Optional[str] = None) -> str:
        if not entity_slug:
            entity_slug = self.entity_model.slug
        return reverse(
            viewname="django_ledger:closing-entry-action-mark-as-unposted",
            kwargs={"entity_slug": entity_slug, "closing_entry_pk": self.uuid},
        )
    ...
    def update_transactions(
        self, force_update: bool = False, post_closing_entry: bool = True, **kwargs
    ):
    ...
    def get_update_transactions_message(self):
        return _(
            f"Are you sure you want to update all Closing Entry {self.closing_date} transactions? "
    ...
    def get_update_transactions_url(self, entity_slug: Optional[str] = None) -> str:
        if not entity_slug:
            entity_slug = self.entity_model.slug
        return reverse(
            viewname="django_ledger:closing-entry-action-update-txs",
            kwargs={"entity_slug": entity_slug, "closing_entry_pk": self.uuid},
        )
    ...
    def get_delete_message(self):
        return _(
            f"Are you sure you want to delete Closing Entry {self.closing_date}? "
    ...
    def get_delete_url(self, entity_slug: Optional[str] = None) -> str:
        if not entity_slug:
            entity_slug = self.entity_model.slug
        return reverse(
            viewname="django_ledger:closing-entry-action-delete",
            kwargs={"entity_slug": entity_slug, "closing_entry_pk": self.uuid},
        )
...
class ClosingEntryTransactionModelQuerySet(QuerySet):
    ...
    def for_closing_date(
        self, closing_date: date
    ) -> "ClosingEntryTransactionModelQuerySet":
        return self.filter(closing_entry_model__closing_date__exact=closing_date)
...
class ClosingEntryTransactionModelAbstract(CreateUpdateMixIn):
    uuid = models.UUIDField(default=uuid4, editable=False, primary_key=True)
    closing_entry_model = models.ForeignKey(
        "django_ledger.ClosingEntryModel", on_delete=models.CASCADE
    )
    ...
    tx_type = models.CharField(
        choices=TransactionModel.TX_TYPE,
        max_length=10,
        verbose_name=_("Transaction Type"),
    )
    balance = models.DecimalField(
        verbose_name=_("Closing Entry Balance"),
        max_digits=20,
        decimal_places=6,
        validators=[MinValueValidator(limit_value=Decimal("0.00"))],
    )

    objects = ClosingEntryTransactionModelManager.from_queryset(
        queryset_class=ClosingEntryTransactionModelQuerySet
    )()
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                fields=["closing_entry_model", "account_model"],
...
def closingentrytransactionmodel_presave(
    instance: ClosingEntryTransactionModel, **kwargs
):
    instance.adjust_tx_type_for_negative_balance()


pre_save.connect(
    closingentrytransactionmodel_presave, sender=ClosingEntryTransactionModel
)
