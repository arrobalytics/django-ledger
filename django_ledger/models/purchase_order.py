...
from django_ledger.models.mixins import (CreateUpdateMixIn, ItemizeMixIn,
                                         MarkdownNotesMixIn)
from django_ledger.models.signals import (po_status_approved,
                                          po_status_canceled, po_status_draft,
                                          po_status_fulfilled,
                                          po_status_in_review, po_status_void)
from django_ledger.models.utils import lazy_loader
from django_ledger.settings import (DJANGO_LEDGER_DOCUMENT_NUMBER_PADDING,
                                    DJANGO_LEDGER_PO_NUMBER_PREFIX,
...
class PurchaseOrderModelAbstract(CreateUpdateMixIn, ItemizeMixIn, MarkdownNotesMixIn):
    """
    ...
    po_number = models.SlugField(
        max_length=20, verbose_name=_("Purchase Order Number"), editable=False
    )
    po_title = models.CharField(
        max_length=250,
        verbose_name=_("Purchase Order Title"),
        validators=[
            MinLengthValidator(
                limit_value=5, message=_(f"PO Title must be greater than 5")
            )
        ],
    )
    po_status = models.CharField(
        max_length=10, choices=PO_STATUS, default=PO_STATUS[0][0]
    )
    ...
    po_amount_received = models.DecimalField(
        default=0, decimal_places=2, max_digits=20, verbose_name=_("Received Amount")
    )
    entity = models.ForeignKey(
        "django_ledger.EntityModel", on_delete=models.CASCADE, verbose_name=_("Entity")
    )
    ...
    date_fulfilled = models.DateField(
        blank=True, null=True, verbose_name=_("Fulfillment Date")
    )
    ...
    objects = PurchaseOrderModelManager.from_queryset(
        queryset_class=PurchaseOrderModelQuerySet
    )()
    ...
    def is_configured(self) -> bool:
        return all([self.entity_id is not None, self.date_draft, self.po_status])
    ...
    def validate_item_transaction_qs(
        ...
        """
        valid = all([i.po_model_id == self.uuid for i in queryset])
        if not valid:
            raise PurchaseOrderModelValidationError(
                f"Invalid queryset. All items must be assigned to PO {self.uuid}"
            )
    ...
    def migrate_itemtxs(self, itemtxs: Dict, operation: str, commit: bool = False):
        ...
        self.clean()
        if commit:
            self.save(update_fields=["po_amount", "po_amount_received", "updated"])
    ...
    def get_itemtxs_data(
        ...
        if aggregate_on_db and isinstance(queryset, ItemTransactionModelQuerySet):
            return queryset, queryset.aggregate(
                po_total_amount__sum=Coalesce(
                    Sum("po_total_amount"), 0.0, output_field=models.FloatField()
                ),
                bill_amount_paid__sum=Coalesce(
                    Sum("bill_model__amount_paid"),
                    0.0,
        ...
        return (
            queryset,
            {
                ...
                "bill_amount_paid__sum": sum(
                    i.bill_model.amount_paid for i in queryset if i.bill_model_id
                ),
    ...
    def update_state(
        ...
        if isinstance(itemtxs_qs, list):
            self.po_amount = round(
                sum(a.po_total_amount for a in itemtxs_qs if not a.is_canceled()), 2
            )
            self.po_amount_received = round(
                sum(a.po_total_amount for a in itemtxs_qs if a.is_received()), 2
            )
        elif isinstance(itemtxs_qs, ItemTransactionModelQuerySet):
            total_po_amount = round(
                sum(i.po_total_amount for i in itemtxs_qs if not i.is_canceled()), 2
            )
            total_received = round(
                sum(i.po_total_amount for i in itemtxs_qs if i.is_received()), 2
            )
    ...
    def can_generate_po_number(self):
        ...
        """
        return all([self.date_draft, not self.po_number])
    ...
    def mark_as_draft(
        ...
        self.clean()
        if commit:
            self.save(update_fields=["po_status", "updated"])
        po_status_draft.send_robust(
    ...
    def get_mark_as_draft_url(self):
        ...
        """
        return reverse(
            "django_ledger:po-action-mark-as-draft",
            kwargs={"entity_slug": self.entity.slug, "po_pk": self.uuid},
        )
    ...
    # REVIEW...
    def mark_as_review(
        self, date_in_review: Optional[date] = None, commit: bool = False, **kwargs
    ):
        """
        ...
        if not itemtxs_qs.count():
            raise PurchaseOrderModelValidationError(
                message="Cannot review a PO without items..."
            )
        ...
        self.clean()
        if commit:
            self.save(update_fields=["po_status", "date_in_review", "updated"])
        po_status_in_review.send_robust(
            sender=self.__class__, instance=self, commited=commit, **kwargs
        )
    ...
    def get_mark_as_review_url(self):
        ...
        """
        return reverse(
            "django_ledger:po-action-mark-as-review",
            kwargs={"entity_slug": self.entity.slug, "po_pk": self.uuid},
        )
    ...
    def mark_as_approved(
        self, date_approved: Optional[date] = None, commit: bool = False, **kwargs
    ):
        """
        ...
        self.clean()
        if commit:
            self.itemtransactionmodel_set.all().update(
                po_item_status=ItemTransactionModel.STATUS_NOT_ORDERED
            )
            self.save(update_fields=["date_approved", "po_status", "updated"])
        po_status_approved.send_robust(
            sender=self.__class__, instance=self, commited=commit, **kwargs
        )
    ...
    def get_mark_as_approved_url(self):
        ...
        """
        return reverse(
            "django_ledger:po-action-mark-as-approved",
            kwargs={"entity_slug": self.entity.slug, "po_pk": self.uuid},
        )
    ...
    def mark_as_canceled(
        ...
        self.clean()
        if commit:
            self.save(update_fields=["po_status", "date_canceled", "updated"])
        po_status_canceled.send_robust(
            sender=self.__class__, instance=self, commited=commit, **kwargs
        )
    ...
    def get_mark_as_canceled_url(self):
        ...
        """
        return reverse(
            "django_ledger:po-action-mark-as-canceled",
            kwargs={"entity_slug": self.entity.slug, "po_pk": self.uuid},
        )
    ...
    def mark_as_fulfilled(
        ...
        po_items: Union[
            ItemTransactionModelQuerySet, List[ItemTransactionModel]
        ] = None,
        commit: bool = False,
        **kwargs,
    ):
        ...
        if not all_items_billed:
            raise PurchaseOrderModelValidationError(
                "All items must be billed before PO can be fulfilled."
            )

        all_bills_paid = all(b.is_paid() for b in bill_models)
        if not all_bills_paid:
            raise PurchaseOrderModelValidationError(
                "All Bills must be paid before PO can be fulfilled."
            )

        all_items_received = all(i.is_received() for i in po_items)
        if not all_items_received:
            raise PurchaseOrderModelValidationError(
                "All items must be received before PO is fulfilled."
            )
        ...
        if commit:
            ...
            self.save(update_fields=["date_fulfilled", "po_status", "updated"])
        po_status_fulfilled.send_robust(
            sender=self.__class__, instance=self, commited=commit, **kwargs
        )
    ...
    def get_mark_as_fulfilled_url(self):
        ...
        """
        return reverse(
            "django_ledger:po-action-mark-as-fulfilled",
            kwargs={"entity_slug": self.entity.slug, "po_pk": self.uuid},
        )
    ...
    # VOID...
    def mark_as_void(
        self, void_date: Optional[date] = None, commit: bool = False, **kwargs
    ):
        """
        Marks PurchaseOrderModel as Fulfilled.
        ...
        if not all(b.is_void() for b in bill_model_qs):
            raise PurchaseOrderModelValidationError(
                "Must void all PO bills before PO can be voided."
            )
        ...
        if commit:
            self.save(update_fields=["void_date", "po_status", "updated"])
        po_status_void.send_robust(
            sender=self.__class__, instance=self, commited=commit, **kwargs
        )
    ...
    def get_mark_as_void_url(self):
        ...
        """
        return reverse(
            "django_ledger:po-action-mark-as-void",
            kwargs={"entity_slug": self.entity.slug, "po_pk": self.uuid},
        )
    ...
    def get_po_bill_queryset(self) -> BillModelQuerySet:
        ...
        return BillModel.objects.filter(
            bill_items__purchaseordermodel__uuid__exact=self.uuid
        )
    ...
    def generate_po_number(self, commit: bool = False) -> str:
        ...
        if self.can_generate_po_number():
            with transaction.atomic(durable=True):
                ...
                self.po_number = (
                    f"{DJANGO_LEDGER_PO_NUMBER_PREFIX}-{state_model.fiscal_year}-{seq}"
                )
