...
from django.db.models import (BooleanField, Case, Count, DecimalField, F,
...
from django_ledger.settings import (DJANGO_LEDGER_MATCH_DAYS_WINDOW,
                ...
                txs_imported_count=Count(
                    ...
                    filter=(
                        ...
                        & (
                            ...
                            | Q(
                                stagedtransactionmodel__matched_transaction_model__isnull=False
                            )
                            | (
                                Q(stagedtransactionmodel__bundle_split=False)
                                & (
                                    Q(
                                        stagedtransactionmodel__split_transaction_set__transaction_model__isnull=False
                                    )
                                    | Q(
...
class ImportJobModelAbstract(CreateUpdateMixIn):
    ...
    def get_ledger_detail_url(self) -> str:
        return reverse(
            ...
            kwargs={"entity_slug": self.entity_slug, "ledger_pk": self.ledger_model_id},
        )
...
class StagedTransactionModelQuerySet(QuerySet):
    ...
    def for_entity(
        self, entity_model: "Union[EntityModel, UUID, str]"
        ...
        if isinstance(entity_model, UUID):
            ...
            return self.filter(
                import_job__ledger_model__entity__slug__exact=entity_model
            )
    ...
    def for_import_job(
        self, import_job_model: "Union[ImportJobModel | UUID]"
    ) -> "StagedTransactionModelQuerySet":
        """
    ...
    def is_pending(self):
        ...
        is_parent_with_pending_children = Q(
            parent__isnull=True,
            bundle_split=False,
            children_mapping_done=False,
            children_import_pending_count__gt=0,
        )
        ...
        matched_transactions = Q(
            transaction_model__isnull=True,
            matched_transaction_model__isnull=False,
        ...
        return self.filter(
            ...
            | Q(
                transaction_model__isnull=True,
                matched_transaction_model__isnull=True,
    ...
    def is_imported(self):
        ...
        """
        parents_with_imported_children = Q(
            parent__isnull=True, bundle_split=False, imported_count__gt=0
        )
        matched_transactions = Q(
            transaction_model__isnull=True,
            matched_transaction_model__isnull=False,
    ...
    def get_queryset(self) -> StagedTransactionModelQuerySet:
        ...
        return (
            ...
            .annotate(
                ...
                import_account_uuid=F(
                    "import_job__bank_account_model__account_model_id"
                ),
                children_count=Count(F("split_transaction_set"), distinct=True),
                children_mapped_count=Count(
                    "split_transaction_set__account_model__uuid", distinct=True
                ),
                imported_count=Count(
                    ...
                    filter=(
                        ...
                        | Q(
                            split_transaction_set__matched_transaction_model_id__isnull=False
                        )
                    ),
                ),
            ...
            .annotate(
                _matches_found=Count(
                    ...
                    filter=(
                        Q(
                            import_job__bank_account_model__account_model__transactionmodel__amount__exact=F(
                                "amount"
                            )
                        )
                        & Q(
...
class StagedTransactionModelAbstract(CreateUpdateMixIn):
    ...
    objects = StagedTransactionModelManager.from_queryset(
        queryset_class=StagedTransactionModelQuerySet
    )()
        ...
        return [
            {
                ...
                "amount": abs(
                    self.amount if not self.is_children() else self.amount_split
                ),
                "amount_staged": self.amount
                if not self.is_children()
                else self.amount_split,
                "unit_model": self.unit_model,
                "tx_type": CREDIT
                if not (self.amount if not self.is_children() else self.amount_split)
                < 0.00
                else DEBIT,
                "description": self.name,
                "staged_tx_model": self,
    ...
    def is_pending(self) -> bool:
        ...
        children_pending = all(
            [
                ...
                self.split_transaction_set.filter(
                    Q(transaction_model__isnull=True)
                    & Q(matched_transaction_model__isnull=True)
                ).exists(),
            ]
        )
    ...
    def can_have_account(self) -> bool:
        ...
        """
        return any(
            [all([self.is_parent(), not self.has_children()]), self.is_children()]
        )
    ...
    def migrate_receipt(
        self, receipt_date: Optional[date | datetime] = None, split_amount: bool = False
    ):
    ...
    def generate_receipt_model(
        ...
        if commit:
            receipt_model.configure(
                ...
                if self.is_expense() or self.is_debt_payment()
                else None,
    ...
    def unmatch(self, raise_exception: bool = True, commit: bool = True):
        ...
        if not self.can_unmatch():
            if raise_exception:
                raise StagedTransactionModelValidationError(
                    message=_(
                        "Nothing to unmatch or cannot unmatch this staged transaction."
                    )
                )
            return
    ...
    def get_update_url(self):
        return reverse(
            ...
            kwargs={
                "entity_slug": self.entity_slug,
                "job_pk": self.import_job_id,
                "staged_tx_pk": self.uuid,
            },
        )
