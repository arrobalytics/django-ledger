...
from django.forms import (BaseModelFormSet, CheckboxInput, HiddenInput,
                          ModelForm, NumberInput, Select, Textarea, TextInput,
                          ValidationError, modelformset_factory)
from django.utils.translation import gettext_lazy as _

from django_ledger.io import (GROUP_DEBT_PAYMENT, GROUP_EXPENSES, GROUP_INCOME,
                              GROUP_TRANSFERS)
from django_ledger.models import (EntityModel, ImportJobModel,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.STAGED_TX_MODEL:
            if isinstance(self.STAGED_TX_MODEL, StagedTransactionModel):
                staged_txs_qs = staged_txs_qs.filter(
                    Q(uuid__exact=self.STAGED_TX_MODEL.uuid)
                    | Q(parent__uuid__exact=self.STAGED_TX_MODEL.uuid)
                )
            elif isinstance(self.STAGED_TX_MODEL, UUID):
                staged_txs_qs = staged_txs_qs.filter(
                    Q(uuid__exact=self.STAGED_TX_MODEL)
                    | Q(parent__uuid__exact=self.STAGED_TX_MODEL)
                )
            else:
        ...
        self.account_model_qs = (
            self.ENTITY_MODEL.get_coa_accounts()
            .available()
        ...
        self.VENDOR_MODEL_QS = entity_model.vendormodel_set.visible().order_by(
            "vendor_name"
        )
        self.CUSTOMER_MODEL_QS = entity_model.customermodel_set.visible().order_by(
            "customer_name"
        )
        ...
        self.FORMS_BY_ID = {
            f.instance.uuid: f
            for f in self.forms
            if getattr(f, "instance", None) and getattr(f.instance, "uuid", None)
        }

        form_children = [
            (f.instance.parent_id, f.instance.uuid)
            for f in self.forms
        ...
        self.FORM_CHILDREN = {
            g: list(j[1] for j in p)
            for g, p in groupby(form_children, key=lambda i: i[0])
        }
...
class StagedTransactionModelForm(ModelForm):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.TRANSFER_ACCOUNT_CHOICES = (
            self.BASE_FORMSET.ACCOUNT_MODEL_TRANSFERS_CHOICES
        )
        self.DEBT_PAYDOWN_ACCOUNT_CHOICES = (
            self.BASE_FORMSET.ACCOUNT_MODEL_DEBT_PAYMENT_CHOICES
        )
        ...
        if staged_tx_model:
            ...
            if (
                any([staged_tx_model.is_transfer(), staged_tx_model.is_debt_payment()])
                and candidates_qs.exists()
            ):
                self.fields["matched_transaction_model"].queryset = candidates_qs
                if candidates_qs.count() == 1:
                    self.fields[
                        "matched_transaction_model"
                    ].initial = candidates_qs.first()
            else:
    ...
    def clean(self):
        ...
        if all(
            [
                ...
                any(
                    [staged_txs_model.is_transfer(), staged_txs_model.is_debt_payment()]
                ),
            ]
        ):
            ...
            if candidates_qs.count() > 1 and selected is None:
                raise ValidationError(
                    message=_(
                        "Multiple matches found. Please select a transaction to match."
                    )
                )
            if candidates_qs.count() == 1 and selected is None:
    ...
    class Meta:
        ...
        help_texts = {
            "bundle_split": _(
                "If checked, maps all splits to the same unit & receipt."
            ),
            "amount_split": _(
                "All splits amount must total parent transaction amount."
            ),
            "account_model": _(
                "The CoA account where the transaction will be imported."
            ),
            "matched_transaction": _(
                "The transaction will be matched against the selected transaction."
            ),
        }
        widgets = {
            ...
            "matched_transaction_model": Select(
                attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}
            ),
