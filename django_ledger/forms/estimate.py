...
from django.forms import (BaseModelFormSet, ModelForm, Select, Textarea,
                          TextInput, modelformset_factory)
from django.utils.translation import gettext_lazy as _

from django_ledger.models import (CustomerModel, EntityUnitModel, ItemModel,
...
class EstimateItemModelForm(ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "ce_unit_cost_estimate": TextInput(
                attrs={
                    "class": DJANGO_LEDGER_FORM_INPUT_CLASSES + " is-small",
                }
            ),
            "ce_unit_revenue_estimate": TextInput(
                attrs={
                    "class": DJANGO_LEDGER_FORM_INPUT_CLASSES + " is-small",
                }
            ),
            "ce_quantity": TextInput(
                attrs={
                    "class": DJANGO_LEDGER_FORM_INPUT_CLASSES + " is-small",
                }
            ),
...
class BaseEstimateItemModelFormset(BaseModelFormSet):
    def __init__(
        ...
        *args,
        entity_slug,
        user_model,
        customer_job_model: EstimateModel,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self.USER_MODEL = user_model
        ...
        items_qs = ItemModel.objects.for_estimate(entity_model=self.ENTITY_SLUG)
