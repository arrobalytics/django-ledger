...
from django.forms import (BaseModelFormSet, BooleanField, DateInput, ModelForm,
                          Select, Textarea, TextInput, ValidationError,
                          modelformset_factory)
from django.utils.translation import gettext_lazy as _

from django_ledger.models import (EntityUnitModel, ItemModel,
                                  ItemTransactionModel, PurchaseOrderModel)
...
class PurchaseOrderModelCreateForm(ModelForm):
    ...
    class Meta:
        ...
        labels = {"for_inventory": _("Is this an inventory purchase?")}
...
class PurchaseOrderItemTransactionForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "po_item_status": Select(
                attrs={
                    "class": DJANGO_LEDGER_FORM_INPUT_CLASSES + " is-small",
                }
            ),
            "po_unit_cost": TextInput(
                attrs={
                    "class": DJANGO_LEDGER_FORM_INPUT_CLASSES + " is-small",
                }
            ),
            "po_quantity": TextInput(
                attrs={
                    "class": DJANGO_LEDGER_FORM_INPUT_CLASSES + " is-small",
                }
            ),
...
class BasePurchaseOrderItemFormset(BaseModelFormSet):
    def __init__(
        self, *args, entity_slug, user_model, po_model: PurchaseOrderModel, **kwargs
    ):
        super().__init__(*args, **kwargs)
        self.USER_MODEL = user_model
        self.ENTITY_SLUG = entity_slug
        ...
        unit_qs = EntityUnitModel.objects.for_entity(entity_model=self.ENTITY_SLUG)
        ...
        for form in self.forms:
            ...
            if not self.PO_MODEL.can_edit_items():
                ...
                if not self.PO_MODEL.is_approved() or self.PO_MODEL.is_fulfilled():
                    ...
                    form.fields["po_item_status"].widget.attrs[
                        "class"
                    ] += form.instance.get_status_css_class()
            # PO is Draft
            else:
                form.fields["po_item_status"].disabled = True
