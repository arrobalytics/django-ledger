...
from django_ledger.io.roles import (ASSET_CA_INVENTORY, GROUP_COGS,
                                    GROUP_EXPENSES, GROUP_INCOME)
...
class UnitOfMeasureModelCreateForm(ModelForm):
    ...
    class Meta:
        ...
        fields = ["name", "unit_abbr", "is_active"]
        widgets = {
            "name": TextInput(attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}),
            "unit_abbr": TextInput(attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}),
        }
...
class ProductCreateForm(ModelForm):
    ...
    def __init__(self, entity_slug: str, user_model, *args, **kwargs):
        ...
        self.fields["earnings_account"].queryset = accounts_qs.filter(
            role__in=GROUP_INCOME
        )
        self.fields["cogs_account"].queryset = accounts_qs.filter(role__in=GROUP_COGS)
        self.fields["inventory_account"].queryset = accounts_qs.filter(
            role__in=[ASSET_CA_INVENTORY]
        )
    ...
    class Meta:
        ...
        widgets = {
            "name": TextInput(attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}),
            "uom": Select(attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}),
            "earnings_account": Select(
                attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}
            ),
            "cogs_account": Select(attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}),
            "inventory_account": Select(
                attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}
            ),
            "sku": TextInput(attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}),
            "upc": TextInput(attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}),
            "item_id": TextInput(attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}),
            "item_type": Select(attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}),
            "default_amount": TextInput(
                attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}
            ),
        }
        labels = {"name": _("Product Name"), "item_type": _("Product Type")}
...
class ServiceCreateForm(ModelForm):
    ...
    def __init__(self, entity_slug: str, user_model, *args, **kwargs):
        ...
        accounts_qs = (
            AccountModel.objects.for_entity(entity_model=self.ENTITY_SLUG)
            .with_roles(
    ...
    class Meta:
        ...
        labels = {"name": _("Service Name"), "sold_as_unit": _("Sold as Unit?")}
...
class ExpenseItemCreateForm(ModelForm):
    def __init__(self, entity_slug: str, user_model, *args, **kwargs):
        ...
        accounts_qs = (
            AccountModel.objects.for_entity(entity_model=self.ENTITY_SLUG)
            .with_roles(roles=GROUP_EXPENSES)
            .active()
        )

        self.fields["expense_account"].queryset = accounts_qs.filter(
