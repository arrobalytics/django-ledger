from django.forms import (BaseModelFormSet, CheckboxInput, DateInput,
                          DateTimeInput, ModelForm, Select, Textarea,
                          TextInput, ValidationError, modelformset_factory)
...
from django_ledger.models import (AccountModel, BillModel, EntityModel,
                                  ...
                                  ItemTransactionModel, VendorModel)
...
class BaseBillModelUpdateForm(BillModelCreateForm):
    def __init__(self, *args, entity_model, user_model, **kwargs):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "cash_account": Select(
                attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES + " is-danger"}
            ),
            "prepaid_account": Select(
                attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES + " is-danger"}
            ),
...
class DraftBillModelUpdateForm(BaseBillModelUpdateForm):
    class Meta(BaseBillModelUpdateForm.Meta):
        fields = ["vendor", "terms", "xref", "markdown_notes"]


class InReviewBillModelUpdateForm(BaseBillModelUpdateForm):
    class Meta(BaseBillModelUpdateForm.Meta):
        fields = ["xref", "markdown_notes"]
        ...
        if itemtxs_model.po_model is not None:
            ...
            if quantity > itemtxs_model.po_quantity:
                raise ValidationError(
                    f"Cannot bill more than {itemtxs_model.po_quantity} authorized."
                )
        return cleaned_data
