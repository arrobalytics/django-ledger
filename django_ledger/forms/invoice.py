...
from django.forms import (CheckboxInput, DateInput, DateTimeInput, ModelForm,
                          Select, Textarea, TextInput, modelformset_factory)
from django.forms.models import BaseModelFormSet
from django.utils.translation import gettext_lazy as _

from django_ledger.io.roles import (ASSET_CA_CASH, ASSET_CA_RECEIVABLES,
                                    LIABILITY_CL_DEFERRED_REVENUE)
from django_ledger.models import (AccountModel, CustomerModel, InvoiceModel,
...
class InvoiceModelCreateForEstimateForm(ModelForm):
    ...
    def get_accounts_queryset(self):
        if all(
            ...
            self.fields["prepaid_account"].queryset = account_qs.filter(
                role__exact=ASSET_CA_RECEIVABLES
            )
            self.fields["unearned_account"].queryset = account_qs.filter(
                role__exact=LIABILITY_CL_DEFERRED_REVENUE
            )

    class Meta:
        model = InvoiceModel
        fields = ["terms", "cash_account", "prepaid_account", "unearned_account"]
        labels = {
        ...
        widgets = {
            ...
            "amount_due": TextInput(
                attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES, "placeholder": "$$$"}
            ),
...
class BaseInvoiceModelUpdateForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "amount_paid": TextInput(attrs={"class": DJANGO_LEDGER_FORM_INPUT_CLASSES}),
...
class ApprovedInvoiceModelUpdateForm(BaseInvoiceModelUpdateForm):
    class Meta(BaseInvoiceModelUpdateForm.Meta):
        fields = ["amount_paid", "markdown_notes"]


class AccruedAndApprovedInvoiceModelUpdateForm(BaseInvoiceModelUpdateForm):
    class Meta(BaseInvoiceModelUpdateForm.Meta):
        fields = ["progress", "amount_paid", "markdown_notes"]
...
class InvoiceItemForm(ModelForm):
    def __init__(self, *args, entity_slug, user_model, invoice_model, **kwargs):
        ...
        if self.instance.item_model_id:
            ...
            if item_model.for_inventory and quantity > item_model.inventory_received:
                raise ValidationError(
                    f"Cannot invoice more than {item_model.inventory_received} units available."
                )
        return cleaned_data

    class Meta:
        model = ItemTransactionModel
        fields = ["item_model", "unit_cost", "quantity"]
        widgets = {
            "item_model": Select(
                attrs={
...
class BaseInvoiceItemTransactionFormset(BaseModelFormSet):
    def __init__(self, *args, entity_slug, user_model, invoice_model, **kwargs):
        ...
        items_qs = ItemModel.objects.for_invoice(entity_model=self.ENTITY_SLUG)
    ...
    def get_queryset(self):
        if not self.queryset:
            self.queryset = ItemTransactionModel.objects.for_invoice(
                entity_model=self.ENTITY_SLUG, invoice_pk=self.INVOICE_MODEL.uuid
            )
        else:
