...
class EntityUnitModelInLine(TabularInline):
    ...
    fields = ["slug", "name", "document_prefix", "active", "hidden"]
...
class EntityModelAdmin(ModelAdmin):
    ...
    readonly_fields = ["depth", "path", "numchild", "last_closing_date", "default_coa"]
    fieldsets = [
        (
            "Entity Information",
            {
        ...
        ("Chart of Accounts", {"fields": ["default_coa"]}),
    ]
    inlines = [ChartOfAccountsInLine, EntityUnitModelInLine, EntityManagementInLine]
    actions = ["add_code_of_accounts", "populate_random_data"]
    ...
    def add_ledger_link(self, obj):
        ...
        return format_html(
            '<a class="addlink" href="{url}?entity_slug={slug}">Add Ledger</a>',
            url=add_ledger_url,
            slug=obj.slug,
        )
    ...
    def balance_sheet_link(self, obj: EntityModel):
        add_ledger_url = reverse(
            viewname="django_ledger:entity-bs", kwargs={"entity_slug": obj.slug}
        )
        return format_html(
            '<a class="viewlink" href="{url}">View</a>',
            url=add_ledger_url,
            slug=obj.slug,
        )
    ...
    def income_statement_link(self, obj: EntityModel):
        add_ledger_url = reverse(
            viewname="django_ledger:entity-ic", kwargs={"entity_slug": obj.slug}
        )
        return format_html(
            '<a class="viewlink" href="{url}">View</a>',
            url=add_ledger_url,
    ...
    def cash_flow_statement_link(self, obj: EntityModel):
        add_ledger_url = reverse(
            viewname="django_ledger:entity-cf", kwargs={"entity_slug": obj.slug}
        )
        return format_html(
            '<a class="viewlink" href="{url}">View</a>',
            url=add_ledger_url,
    ...
    def dashboard_link(self, obj: EntityModel):
        add_ledger_url = reverse(
            viewname="django_ledger:entity-dashboard", kwargs={"entity_slug": obj.slug}
        )
        return format_html(
            '<a class="viewlink" href="{url}">View</a>',
            url=add_ledger_url,
    ...
    def populate_random_data(self, request, queryset):
        for entity_model in queryset:
            ...
            entity_model.populate_random_data(
                start_date=start_date, days_forward=180, tx_quantity=25
            )
