...
from django_ledger.models import (EntityModel, JournalEntryModel, LedgerModel,
...
class JournalEntryModelInLine(TabularInline):
    ...
    def get_queryset(self, request):
        ...
        return qs.annotate(txs_count=Count("transactionmodel")).select_related(
    ...
    def has_change_permission(self, request, obj=None):
        if obj:
            return all(
                [not obj.is_locked(), super().has_change_permission(request, obj)]
            )
        return super().has_change_permission(request, obj)

    def has_delete_permission(self, request, obj=None):
        if obj:
            return all([obj.can_delete(), super().has_delete_permission(request, obj)])
...
class LedgerModelAdmin(ModelAdmin):
    readonly_fields = ["entity", "posted", "locked"]
    list_filter = ["posted", "locked", "entity__name"]
    list_display = [
        "name",
        "ledger_xid",
        "is_posted",
    ...
    actions = ["post", "unpost", "lock", "unlock"]
    inlines = [JournalEntryModelInLine]
    ...
    def get_fieldsets(self, request, obj=None):
        if obj is None:
            return [("", {"fields": ["name", "hidden", "additional_info"]})]
        return [("", {"fields": ["entity", "name", "hidden", "additional_info"]})]
    ...
    def post(self, request, queryset):
        ...
        queryset.bulk_update(objs=queryset, fields=["posted", "updated"])
    ...
    def lock(self, request, queryset):
        ...
        queryset.bulk_update(objs=queryset, fields=["locked", "updated"])
