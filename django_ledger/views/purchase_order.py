...
from django_ledger.forms.purchase_order import (
    ApprovedPurchaseOrderModelUpdateForm, BasePurchaseOrderModelUpdateForm,
    DraftPurchaseOrderModelUpdateForm, PurchaseOrderModelCreateForm,
...
class PurchaseOrderModelListView(
    PurchaseOrderModelModelViewQuerySetMixIn, ArchiveIndexView
):
...
class PurchaseOrderModelCreateView(
    PurchaseOrderModelModelViewQuerySetMixIn, CreateView
):
    ...
    def get(self, request, entity_slug, **kwargs):
        response = super(PurchaseOrderModelCreateView, self).get(
            request, entity_slug, **kwargs
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.for_estimate:
            context["form_action_url"] = reverse(
                "django_ledger:po-create-estimate",
                kwargs={
    ...
    def get_success_url(self):
        ...
        if self.for_estimate:
            ...
            return reverse(
                ...
                kwargs={"entity_slug": self.kwargs["entity_slug"], "ce_pk": ce_pk},
            )
        return reverse("django_ledger:po-list", kwargs={"entity_slug": entity_slug})


class PurchaseOrderModelUpdateView(
    PurchaseOrderModelModelViewQuerySetMixIn, UpdateView
):
    ...
    def get_context_data(self, itemtxs_formset=None, **kwargs):
        ...
        if po_model.is_contract_bound():
            ...
            context["ce_cost_estimate__sum"] = sum(
                i["ce_cost_estimate__sum"] for i in ce_itemtxs_agg
            )
...
class PurchaseOrderModelDetailView(
    PurchaseOrderModelModelViewQuerySetMixIn, DetailView
):
    slug_url_kwarg = "po_pk"
    slug_field = "uuid"
    context_object_name = "po_model"
...
class PurchaseOrderModelDeleteView(
    PurchaseOrderModelModelViewQuerySetMixIn, DeleteView
):
    slug_url_kwarg = "po_pk"
    slug_field = "uuid"
    context_object_name = "po_model"
    template_name = "django_ledger/purchase_order/po_delete.html"
    extra_context = {"hide_menu": True, "header_subtitle_icon": "uil:bill"}
...
class BasePurchaseOrderActionActionView(
    PurchaseOrderModelModelViewQuerySetMixIn, RedirectView, SingleObjectMixin
):
