...
from django_ledger.forms.journal_entry import (JournalEntryModelCannotEditForm,
...
class JournalEntryModelModelBaseView(DjangoLedgerSecurityMixIn):
    ...
    def get_queryset(self):
        if self.queryset is None:
            ...
            journal_entry_queryset = ledger_model.journal_entries.select_related(
                "entity_unit", "ledger", "ledger__entity"
            ).order_by("-timestamp")
    ...
    def get_ledger_model(self) -> LedgerModel:
        if self.ledger_model is None:
            ...
            self.ledger_model = entity_model.get_ledgers().get(
                uuid__exact=self.kwargs["ledger_pk"]
            )
...
class JournalEntryCreateView(JournalEntryModelModelBaseView, CreateView):
    ...
    def get_initial(self):
        return {"timestamp": get_localtime()}
...
class JournalEntryListView(JournalEntryModelModelBaseView, ArchiveIndexView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context[
            "header_subtitle"
        ] = f"{entity_model.name} | Ledger: {ledger_model.name}"
        ...
        if ledger_model.is_locked():
            messages.add_message(
                self.request,
                message=_(
                    "Locked Journal Entry. Must unlock ledger to add new Journal Entries."
                ),
...
class JournalEntryModelTXSDetailView(JournalEntryModelModelBaseView, DetailView):
    ...
    def get_queryset(self):
        ...
        return qs.prefetch_related(
            "transactionmodel_set", "transactionmodel_set__account"
        )
...
class BaseJournalEntryActionView(
    JournalEntryModelModelBaseView, RedirectView, SingleObjectMixin
):
    http_method_names = ["get"]
    pk_url_kwarg = "je_pk"
    action_name = None
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            getattr(je_model, self.action_name)(
                commit=self.commit, verify=True, raise_exception=True, **kwargs
            )
