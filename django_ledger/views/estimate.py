...
from django_ledger.forms.estimate import (BaseEstimateModelUpdateForm,
...
class EstimateModelModelViewQuerySetMixIn:
    ...
    def get_queryset(self):
        if self.queryset is None:
            ...
            self.queryset = entity_model.estimatemodel_set.select_related(
                "customer", "entity"
            )
        return super().get_queryset()


class EstimateModelListView(
    DjangoLedgerSecurityMixIn, EstimateModelModelViewQuerySetMixIn, ArchiveIndexView
):
...
class EstimateModelCreateView(
    DjangoLedgerSecurityMixIn, EstimateModelModelViewQuerySetMixIn, CreateView
):
    ...
    def get_success_url(self):
        ...
        return reverse(
            ...
            kwargs={"entity_slug": self.kwargs["entity_slug"], "ce_pk": cj_model.uuid},
        )
...
class EstimateModelDetailView(
    DjangoLedgerSecurityMixIn, EstimateModelModelViewQuerySetMixIn, DetailView
):
    pk_url_kwarg = "ce_pk"
    ...
    def get_context_data(self, **kwargs):
        ...
        # PO Model Queryset...
        po_qs = (
            ce_model.purchaseordermodel_set.for_entity(
                entity_model=self.kwargs["entity_slug"]
            )
            if ce_model.is_approved()
            else ce_model.purchaseordermodel_set.none()
        )
        context["estimate_po_model_queryset"] = po_qs

        invoice_qs = (
            ce_model.invoicemodel_set.for_entity(
                entity_model=self.kwargs["entity_slug"]
            )
            if ce_model.is_approved()
            else ce_model.invoicemodel_set.none()
        )
        context["estimate_invoice_model_queryset"] = invoice_qs

        bill_qs = (
            ce_model.billmodel_set.for_entity(entity_model=self.kwargs["entity_slug"])
            if ce_model.is_approved()
            else ce_model.billmodel_set.none()
        )
        ...
        if ce_model.is_contract():
            context["contract_progress"] = ce_model.get_contract_summary(
                po_qs=po_qs, invoice_qs=invoice_qs, bill_qs=bill_qs
            )
...
class EstimateModelUpdateView(
    DjangoLedgerSecurityMixIn, EstimateModelModelViewQuerySetMixIn, UpdateView
):
    ...
    def get(self, request, entity_slug, ce_pk, *args, **kwargs):
        if self.action_update_items:
            return HttpResponseRedirect(
                redirect_to=reverse(
                    "django_ledger:customer-estimate-update",
                    kwargs={"entity_slug": entity_slug, "ce_pk": ce_pk},
                )
            )
        return super(EstimateModelUpdateView, self).get(
            request, entity_slug, ce_pk, *args, **kwargs
        )
