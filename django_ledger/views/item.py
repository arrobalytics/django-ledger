...
from django_ledger.forms.item import (ExpenseItemCreateForm,
...
class UnitOfMeasureModelCreateView(UnitOfMeasureModelModelBaseView, CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "django_ledger:uom-list", kwargs={"entity_slug": self.kwargs["entity_slug"]}
        )
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            add_message(
                ...
                level=ERROR,
                message=_(
                    f"User {self.request.user.username} cannot access entity {entity_slug}."
                ),
...
class ProductItemModelModelBaseView(DjangoLedgerSecurityMixIn):
    ...
    def get_queryset(self):
        if not self.queryset:
            entity_model: EntityModel = self.get_authorized_entity_instance()
            self.queryset = (
                entity_model.itemmodel_set.products()
                .select_related(
                    "earnings_account", "cogs_account", "inventory_account", "uom"
                )
                .order_by("-updated")
            )
...
class ServiceItemModelModelBaseView(DjangoLedgerSecurityMixIn):
    ...
    def get_queryset(self):
        if not self.queryset:
            entity_model: EntityModel = self.get_authorized_entity_instance()
            self.queryset = (
                entity_model.itemmodel_set.services()
                .select_related(
                    "earnings_account", "cogs_account", "inventory_account", "uom"
                )
                .order_by("-updated")
...
class ExpenseItemItemModelModelBaseView(DjangoLedgerSecurityMixIn):
    ...
    def get_queryset(self):
        if not self.queryset:
            entity_model: EntityModel = self.get_authorized_entity_instance()
            self.queryset = (
                entity_model.itemmodel_set.expenses()
                .select_related("expense_account", "uom")
                .order_by("-updated")
            )
        return super().get_queryset()
...
class InventoryItemItemModelModelBaseView(DjangoLedgerSecurityMixIn):
    ...
    def get_queryset(self):
        if not self.queryset:
            entity_model: EntityModel = self.get_authorized_entity_instance()
            self.queryset = (
                entity_model.itemmodel_set.inventory_wip()
                .select_related("inventory_account", "cogs_account", "uom")
                .order_by("-updated")
            )
        return super().get_queryset()
